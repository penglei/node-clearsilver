...
unsigned int var_ui_int_lookup (CSPARSE *parse, char *name)
{
	char *vs;
...
static NEOERR *parse_expr2 (CSPARSE *parse, CSTOKEN *tokens, int ntokens, int lvalue, CSARG *arg)
{
	...
	if (ntokens == 1)
	{
		...
		if (tokens[0].type & CS_TYPES)
		{
			...
			//if (tokens[0].len >= 0) size_t(unsinged int) nerver smaller than zero
...
key:需要替换的关键字
replace_key:替换后的key
如果key在s的开头，用replace_key替换key，s的子串保存到ans中
*/
static int StringFirstwordReplace(char *s, const char *key, const char *replace_key, char **ans )
{
	int keylen = strlen(key);	//key  需要匹配的关键字
	int srclen = strlen(s);	// s  源字符串
	int rkeylen = strlen(replace_key);
	int dstlen = srclen + rkeylen;

	if(((*ans) = (char*)malloc(dstlen)) == NULL)
	{
		return -1;
	}

	//if key is ilegal,No need replace
	if(keylen>srclen || keylen==0 || srclen==0 || rkeylen==0)
	{		
		memset(*ans,0,dstlen);
		strncpy(*ans,s,srclen);
		return 0;
	}

	//replace the words key in s's first place, now keylen<=srclen
	int iMatch = 1;
	int i;
	for(i=0;i<keylen;++i)
	{
		//judge if A to a match
		if(s[i]>='a' && s[i]<='z' && (s[i]-32)== key[i])
			continue;
		else if(s[i]>='A' && s[i]<='Z' && (s[i]+32)== key[i])
			continue;
		if(s[i] != key[i])
		{
			iMatch = 0;
	...
	{
		memset(*ans,0,dstlen);
		strncpy(*ans,replace_key,rkeylen);
		strncpy(*ans+rkeylen,s+keylen,srclen-keylen);
	}
	else
	{
		memset(*ans,0,dstlen);
...
s:源字符串
key:需要匹配的关键字
如果key在s的开头，把key截去，s的子串保存到ans中
*/
static int StringFirstwordFilter(char *s, const char *key, char **ans )
{
	int keylen = strlen(key);	//key  需要匹配的关键字
	int srclen = strlen(s);	// s  源字符串
	int dstlen = srclen + 10;
	...
	while(1)
	{
		...
		//if key is ilegal,No need filter
		if(keylen>srclen || keylen==0 || srclen==0)
		{		
			if(iHaveContent == 0)
			{
				memset(*ans,0,dstlen);
		...
		//filter the words key in s's first place, now keylen<=srclen
		int iMatch = 1;
		int i;
		for(i=0;i<keylen;++i)
		{
		...
		{
			memset(*ans,0,dstlen);
			strncpy(*ans,s+keylen,srclen-keylen);

			//prepare for next loop
			memset(s, 0, srclen);
			strncpy(s, *ans, srclen-keylen);
...
static NEOERR * _builtin_string_firstword_replace(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	char *s = NULL;
	...
	err = cs_arg_parse(parse, args, "sss", &s, &s_key, &s_replace);
	if (err) {
		return nerr_pass(err);
	}
	/* If null, return empty string */
	if (s==NULL || s_key==NULL || s_replace==NULL) {
		return STATUS_OK;
	}

	//replace string data
	if(StringFirstwordReplace(s, s_key, s_replace, &ans ) != 0 )
	{
		return nerr_raise(NERR_NOMEM, "Unable to allocate memory for string slice");
	}
...
static NEOERR * _builtin_string_firstword_filter(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	char *s = NULL;
	...
	err = cs_arg_parse(parse, args, "ss", &s, &s_key);
	if (err) {
		return nerr_pass(err);
	}
	/* If null, return empty string */
	if (s==NULL || s_key==NULL) {
		return STATUS_OK;
	}

	//deal string data
	if(StringFirstwordFilter(s, s_key, &ans ) != 0 )
	{
		return nerr_raise(NERR_NOMEM, "Unable to allocate memory for string slice");
	}
...
static NEOERR *cs_init_internal (CSPARSE **parse, HDF *hdf, CSPARSE *parent)
{
	...
	if (parent == NULL)
	{
		static struct _builtin_functions {
			...
			{ "string_firstwords_filter", 2, _builtin_string_firstword_filter},
			{ "string_firstwords_replace", 3, _builtin_string_firstword_replace},
